...
2. **vendor tool:**
   ...
   - update: Update vendor information
     ...
     ```json
     {
       ...
       "action": "update",
       "parameters": {
         "vendorId": "<the current workflow's vendor identifier>",
         "vendorData": {
           "companyName": "extracted from vendor's message",
           "contactPerson": "extracted from vendor's message",
           "email": "extracted from vendor's message",
           "phone": "extracted from vendor's message"
         }
       }
     }
     ```
     
     **Note:** vendorData MUST always be a nested object, never flat fields directly under parameters. Only include fields the vendor actually provided — don't invent values for fields not mentioned.
